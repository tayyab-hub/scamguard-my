...
General SCAMGUARD branding follows supervisor feedback (D20). Preserve the warm palette, typography, spacing and short CSS motion. URL success uses “Submission recorded.” with “No assessment yet.” Message success uses “Analysis completed.” and an evidence-led result panel. Risk labels always include text, confidence is visibly separate, limitations remain available, and no badge implies proof of safety or fraud. Storage and the possibility of administrator-enabled redacted external review are disclosed before Message submission. History uses escaped text, on-demand detail, uniquely identified native disclosure buttons and bounded pagination.

## Post-Task 4 refinement (2026-09-07)

Current Message and URL both display completed assessments; this supersedes the historical URL intake-only wording above. Preserve the approved colour and type tokens. Panels now use 12px radii and restrained layered paper gradients/shadows; section headings and sidebar selection have deliberate tonal separation. Nearly equal desktop Analyse columns make space for results. Result container queries switch summary/metadata columns at 460px of available width; narrow results stack. Semantic risk accents use olive Low, ochre Caution, terracotta Elevated, red High and neutral insufficient, always with categorical text.

Shared result components present an authoritative category heading, advisory risk score, separate confidence, evidence and actions. Message displays existing fusion points; URL uses a labelled ordinal category index, with no within-category or probability claim. Method and limitations are available beside the score. History uses the same components and stored risk labels. See [UI_UX_REFINEMENT.md](UI_UX_REFINEMENT.md) for derivation and reuse details.

Score reveal uses scaleX over 240ms; the text value is immediate. Disclosures reveal with 180ms opacity, and validation/chips use 150ms fades. Native details closes immediately; content is never held for an exit. Existing no-preference-only motion and global reduced-motion override remain in force, including mid-request preference changes. No width/height animation, timer, library, neon or continuous settled-state effect was added. New loading skeletons represent genuine pending submissions only.
