...
export function PreviewNotice({
  ...
  return (
    <div
      className="mb-6 flex flex-wrap items-start gap-3 rounded-lg border border-warning/30 bg-warning-subtle px-5 py-4"
      role="alert"
    >
      <CircleAlert size={18} className="mt-0.5 shrink-0 text-warning" aria-hidden="true" />
      <div className="min-w-0 flex-1 basis-48">
        <p className="text-sm font-medium text-warning">Development preview · Service unavailable</p>
        ...
        {error instanceof ApiError && error.requestId && (
          <p className="mt-2 break-all font-mono text-xs text-muted">Reference: {error.requestId}</p>
        )}
      </div>
