...
const scenarios = [
  ...
  ['https://paypal.com.secure-login.example.net/verify', 'Elevated risk', 'Brand-like token outside its domain'],
  ['https://example.net/login?redirect=https%3A%2F%2Fother.example', 'Caution advised', 'Destination parameter'],
] as const

test('URL scenarios: persisted real results, no navigation to destinations, keyboard and 320px layout', async ({ page }, info) => {
  const errors: string[] = []
  const outside: string[] = []
  page.on('pageerror', e => errors.push(e.message))
  page.on('request', req => { if (!req.url().startsWith('http://127.0.0.1:5175/')) outside.push(req.url()) })
  ...
  for (const [url, risk, evidence] of scenarios) {
    ...
    const responsePromise = page.waitForResponse(r => r.url().endsWith('/api/v1/analyses') && r.request().method() === 'POST')
    ...
    if (risk === 'High risk') {
      await page.setViewportSize({width: info.project.name==='mobile' ? 320 : 1440, height:950})
  ...
  await page.reload()
  await page.getByRole('button', { name: /https:\/\/example.net\/login/ }).click()
  await expect(page.getByText('Destination parameter')).toBeVisible()
  await page.emulateMedia({reducedMotion:'reduce'})
  expect(await page.getByLabel('URL assessment').evaluate(e => getComputedStyle(e).animationName)).toBe('none')
